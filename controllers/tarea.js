...
const ordenT = async (req, res) => {
  ...
  //res.json({ordenws:newArray})

  //console.log(`Insertado con post: `,instance);
  ...
  /*  const { SampleID, TestID, HIS, fact_id, valorresult } = data;
 ...
 exam_id:Number(data.HIS),
     fact_id:data.fact_id,
     resu_exam_valor:Number(data.valorresult),
 };
      const instance= axios.create({
     baseURL: process.env.MARTIN,
     params
 });
 console.log(`Insertado con pwwwwos2323t: `,instance);
       //const response = await axios.post(endpointURL, data);
       //console.log(`Insertado con éxito1: ${JSON.stringify(newData)}`);
       const response= await instance.post();
 console.log(`Insertado con pwwwwost: `,response);
      console.log(`Insertado con éxito: ${response.data}`);
  if (response.data === HIS) {
   let params2={
    // exam_id:`${HIS}`,
     token:process.env.token,
     exam_id:HIS,
 ...
 console.log(`respuesta de update sais`,instance2.data)
 if (instance2.data ==ok || instance2.data =='ok') {
   const updateOrden =await Cabecera_Agen.update({ ESTADO: 2},{where:{HIS:HIS}})
   console.log(`**ORDEN ENVIADA CON EXITO`)
 }
 }  
